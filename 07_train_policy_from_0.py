...
def main():
    ...
    model_save_path = './model_from_0/'
    model_name = '2d-potential'
    saver = tf.train.Saver(var_list=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='discriminator'), max_to_keep=int(max_episode/100))
    ckpt = tf.train.get_checkpoint_state(model_load_path)
    if ckpt and ckpt.model_checkpoint_path:
            print('Found Saved Model.')
            # 669 674 1034 1037 1039
            ckpt_to_restore = ckpt.all_model_checkpoint_paths[833]
    else:
